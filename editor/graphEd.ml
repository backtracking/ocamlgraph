...
let edge v w = mem_edge g v w || mem_edge g w v 
...
let rec draw_edges t pas angle= function
  | [] -> 
      []
  | v :: l -> 
      ...
      (v,tv) :: draw_edges t pas angle l

module H = Hashtbl.Make(V)

let pos = H.create 97

let  draw_graph graph noeud tortue =
  H.clear pos;
  H.add pos noeud (0,tortue);
  tmoveto tortue;
  draw_label noeud;
  let l = succ graph noeud in 
  let l = List.filter (fun x -> x <> noeud ) l in
...
(***
      let lsucc = List.fold_left (fun s v -> Vset.union s (vset_of_list (succ graph v))) Vset.empty l in
      let lsucc = Vset.diff lsucc (vset_of_list l) in
      let lsucc = Vset.remove noeud lsucc in
      let lsucc = Vset.elements lsucc in
      let lsucc = List.map (fun v -> v, List.filter (fun (w,_) -> edge v w) ll) lsucc in
***)      
      List.iter 
	(fun (w,tw) ->	   
