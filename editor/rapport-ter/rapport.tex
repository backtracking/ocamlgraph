...
\newcommand{\miquel}{Alexandre Miquel}
...
d'un plan euclidien, et son bord C correspond à l'horizon.
...
droites sont représentées par des arcs de cercle perpendiculaires à
C comme illustré ci-après par la figure de droite. 
\begin{center}
\end{center}
\begin{center}
...
Ce modèle est doté d'une fonction $\delta$ permettant de calculer la
distance entre deux points $A$ et $B$ du disque de Poincaré que l'on
appel une métrique, et dont la formule est la suivante : 
$$
...
observons un petit être vivant sur ce plan hyperbolique et qu'il
souhaite se déplacer dessus. À chaque pas qu'il fait dans la direction
de l'horizon (le bord du disque pour l'observateur), la distance par
unité de pas qu'il parcours parait de plus en plus petite pour
l'observateur. Ainsi bord du disque est in-atteignable pour ce petit
...
\item
\begin{ocaml}
val advance : turtle -> float -> turtle = <fun>
\end{ocaml}\\
advance qui prend une tortue et un pas d'avancement et renvoi une
nouvelle tortue\\ 
\item
\begin{ocaml}
val turnleft : turtle -> float -> turtle = <fun>
\end{ocaml}\\
turnleft qui prend une tortue et un angle en radian et renvoi une
nouvelle tortue\\ 
\item
\begin{ocaml}
val turnright : turtle -> float -> turtle = <fun>
\end{ocaml}\\
turnright qui prend une tortue et un angle en radian et renvoi une nouvelle
tortue\\
\end{itemize}
...
deux pas de temps identique, la différence de distance parcouru qui
...
Nous nous interessons d'abord au travail effectué par
\miquel~\cite{miquel} sur les arbres qui sont un cas particulier, et
simplifié, de graphes qui n'a qu'une seule source et aucun cycle. Le
problème est biensûr de placer correctement les noeuds et ensuite le
traçage des arêtes est relativement trivial avec l'aide de notre
tortue et de ses fonctions de mouvement.

Pour répartir les noeuds d'un arbre dans le plan hyperbolique, il se
base sur deux propriété de ce plan :
\begin{itemize}
...
Pour arriver à répartir l'espace entre ces n demi-plans. Pour cela, \miquel illustre
...
ses deux pieds).
 ...
 deux à deux disjoints. Pour calculer cette distance d, il utilise la
 formule suivante :
$$
d_n= \mathrm{ath}\left(\mathrm{tan}\left(\frac{\pi}{4}-\frac{\pi}{2n}\right)\right)
$$ 
Donc pour le placement des noeuds, il pose la racine au centre du
...
récursivement pour l'ensemble des noeuds.

Après avoir déterminer où placer les noeuds d'un arbre, on peut
s'intéresser au placement des noeuds d'un graphes qui pose certains
