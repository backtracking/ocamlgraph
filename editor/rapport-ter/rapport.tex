...
graphe. En effet, sur la \figl{dfs-dfs}, on voit une tendance à regrouper les
\noeuds\ et éviter trop de croisement d'arêtes, contrairement à la
\figl{dfs-bfs} qui a tendance, elle, a se faire croiser une grande
...
risque plus grand que certainesse croisent. 
...
Dfs). La \figl{bfs-dfs} montre par contre bien que le Dfs compresse
tout le graphe dans un coin du plan, alors que le Bfs \figs{bfs-bfs}
...
croiser le moins d'arêtes possible. Pour ce faire, on commence tout
d'abord à faire la liste de tous les successeurs d'un noeud. De cette
liste, on construit des listes des composantes fortement
connexes. Ainsi, nous avons toutes les composantes qui sont séparées,
ce qui nous permet de choisir l'ordre d'affichage de ces composantes.
Pour ordonner ces sous listes, on prend pour chaque composante le
\noeud\ de degré minimal et on effectue à partir de lui un parcours en
profondeur. Les listes résultantes de l'ensemble des composantes est
ensuite concaténé pour nous rendre la liste ordonnée des successeur du
\noeud\ étudié.
