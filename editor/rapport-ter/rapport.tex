...
\begin{figure}
 ...
 \includegraphics[scale=0.50]{images/parallels.eps}
  \caption{Théorême des parallèles en géométrie hyperbolique}
  \label{fig:parallels}
\end{figure}
...
\end{figure}
 ...
 On peut voir \figs{lentille1} que pour représenter un
...
La \figl{lentille2} représente grossièrement le passage d'un dessin du
...
\begin{figure}[!b]
  \centering
   \includegraphics[scale=0.35]{images/perpendiculaire.eps}  
...
petit être vivant sur ce plan hyperbolique et qui se déplace dessus. À
...
arbre, c'est-à-dire qu'on affecte à chacun de ses successeurs
immédiats un demi-plan puis, récursivement, on procède à l'affichage
des descendants de chacun de ses successeurs.
Mais il y a cependant deux différences importantes.
En effet, il peut y avoir plusieurs chemins différents menant de
la racine à un \noeud\ donné. Dès lors, un \noeud\ peut déjà avoir été
affiché comme successeur immédiat d'un descendant de la racine déjà
traité, et ne doit donc plus \^etre considéré comme successeur de tout
autre \noeud. Cependant, les ar\^etes correspondantes doivent \^etre
affichées, ce qui amène à ajouter des ar\^etes transversales par
rapport au dessin d'un arbre, c'est-à-dire des ar\^etes
s'affranchissant des frontières entre demi-plans.
...
partir de la racine, qui détermine si un \noeud\ est visité plus ou
...
en profondeur (DFS) et le parcours en largeur (BFS). Nous avons
implanté les deux, ce qui a permis de voir les conséquences d'un tel
choix. En observant les deux figures suivantes, il est clairement
visible que le choix entre les deux n'est pas évident.  Le Dfs semble
moins croiser les arêtes. Le Bfs équilibre bien mieux la
représentation du graphe.  Pour choisir un parcours par défaut, nous
avons préféré nous appuyer sur quelque chose de s\^ur plut\^ot qu'une
chose possible. En effet, dans ce cas, le Dfs croise moins les arêtes,
mais qu'en serait-il dans d'autres cas ; alors que dans tous les cas, le
Bfs équilibrera mieux le graphe.  Le choix s'est donc objectivement
porté sur le parcours Bfs qui permet de voir plus de \noeuds\ d'un seul
coup.

voir la figure~\ref{fig:dfs}

\begin{figure}
\begin{center}
 \includegraphics[scale=0.30]{images/db4_dfs.eps}  \quad
...
\begin{figure}
\begin{center}
 \includegraphics[scale=0.30]{images/parcours_dfs.eps}  \quad
 \includegraphics[scale=0.30]{images/parcours_bfs.eps}  
\end{center}
  \caption{Intér\^et d'un parcours en largeur}
  \label{fig:bfs}
\end{figure}
