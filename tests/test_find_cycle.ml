...
module GGL = Imperative.Graph.ConcreteLabeled(Int)(Int)
...
let () = 
  ...
  GD.add_edge g 2 3 ;
  GD.add_edge g 2 4 ;
  print_list (try try TD.find_cycle g with Not_found -> [] with Not_found -> [])  
...
let () = 
  ...
  GD.add_edge g 1 1 ;
  GD.add_edge g 1 2 ;
  print_list (try TD.find_cycle g with Not_found -> [])  

(* Graph with self-cycle *)
let () = 
  let g = GG.create () in 
  GG.add_vertex g 1 ; 
  GG.add_vertex g 2 ; 
  GG.add_edge g 1 1 ;
  GG.add_edge g 1 2 ;
  print_list (try TG.find_cycle g with Not_found -> [])   
...
(* Graph with paralel-edges *)
let () = 
  let g = GGL.create () in 
  GGL.add_vertex g 1 ; 
  GGL.add_vertex g 2 ; 
  GGL.add_edge_e g (1, 1, 2) ;
  GGL.add_edge_e g (1, 2, 2) ;
  print_list (try TGL.find_cycle g with Not_found -> [])   
