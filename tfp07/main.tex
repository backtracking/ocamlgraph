...
\begin{abstract}
  ...
  etc.---and a large set of graph algorithms written independently of
  the graph data structure. This genericity is obtained through
  massive use of the \ocaml module system and of its functions, the
  so-called \emph{functors}.
\end{abstract}
...
libraries according to the following criteria: the number of graph
data structures; the purely applicative or imperative nature of the
structures; and the ability to apply the provided algorithms to a
user-defined graph data structure.  As we can notice, none of these
libraries gives full satisfaction.  This article introduces
...
independently of the underlying graph data structure. These can then
be applied to the data structures provided by \ocamlgraph itself and
also on user-defined data structures as soon as they implement a
minimal set of functionalities.

Without proper parameterization, such a large set of variants may
easily result into unmanageable code. We avoid this pitfall using the
...
seen the high number of similar but all different implementations. On
the other hand, they are used to write the graph algorithms
independently of the underlying graph data structure, with as much
genericity as possible but also with efficiency in mind.
...
GHz).  The remainder of this paper will explain the above code in more
detail.
...
%
Here we show how to factorize the \ocamlgraph implementation to avoid
such a pitfall.  Section~\ref{interface} describes the common
...
for graphs datatype and two modules \texttt{V} and \texttt{E} for
...
but the user may choose between \emph{concrete} vertices where
vertices and their values are identical and \emph{abstract} vertices
where the values are hidden inside an abstract data type for vertices.
The former allows a more immediate use of the data structure and the
latter allows a more efficient implementation. In particular,
imperative graphs with abstract vertices can be equipped with mutable
marks.

Each graph data structure is provided as a functor, parameterized by
...
signatures of input modules to signatures of output modules.
\begin{figure*}
  \centering
...
form a subset of the operations provided by \ocamlgraph's own data
...
purpose, we build full graphs with $V$ vertices (and thus $E=V(V+1)/2$
...
(that is half a million of edges).  The creation speed observed is
...
2). Deletion is roughly twice as fast as creation.  Regarding memory
consumption, all four data structures roughly use 5 machine words
