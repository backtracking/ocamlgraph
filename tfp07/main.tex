...
parameterization results in unmanageable code.
%
...
observers. Two other signatures (\texttt{P} and \texttt{I})
distinguish the modifiers for respectively persistent and imperative
graphs.

The common signature \texttt{G} includes an abstract type \texttt{t}
for graphs datatype, two modules \texttt{V} and \texttt{E} for
vertices and edges respectively, and all the observers. The latter
include several functions to iterate over vertices and edges, which
are massively used in graph algorithms.  The common signature thus
looks like:
...
user types for vertex labels and possibly edge labels. These 16
